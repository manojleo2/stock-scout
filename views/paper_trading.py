        ...
        df_log = pd.DataFrame([
            {
                ...
                "Lots & Qty": f"{t.get('display_lot_size') // 350 if 'CDSL' in t.get('symbol', '') else t.get('display_lot_size') // 500} Lot(s) ({t.get('display_lot_size')} sh)",
                "Capital Deployed": f"₹{t.get('display_capital', 0):,.2f}",
                "Entry ₹": f"₹{t.get('entry_premium', 0):.2f}",
                "Exit ₹": f"₹{t.get('exit_premium', 0):.2f}" if t.get('exit_premium') else "Pending...",
                "Net P&L (₹)": f"₹{t.get('display_net', 0):+,.2f}" if t.get('display_net') is not None else "Pending...",
                "Return %": f"{t.get('display_ret', 0):+.1f}%" if t.get('display_ret') is not None else "Pending...",
                "Result": t.get("status"),
