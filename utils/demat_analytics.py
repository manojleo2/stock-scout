...
def compute_demat_run_rates(
    ...
    for CDSL with fundamental impact scores.
    """
...
def render_demat_analytics_widget():
    """
    Renders an interactive Demat Additions weekly tracking section for CDSL.
    """
    st.markdown("### 📈 Demat Account Additions: Weekly Run-Rate & CDSL Impact Engine")
    st.caption("Weekly run-rate breakdown and stock impact analysis specifically tailored for CDSL.NS")
    ...
    with st.expander("🎛️ Adjust Weekly Run-Rate Simulation", expanded=False):
        ...
        sim_result = compute_demat_run_rates(monthly_additions_m=sim_monthly, cdsl_share=sim_cdsl_pct)


    # 3. Weekly Summary Analysis Card for CDSL
    st.markdown("#### 📝 Weekly Fundamental Impact for CDSL")
    with st.container(border=True):
        st.markdown(f"""
            <div style='display: flex; justify-content: space-between; align-items: center;'>
                <h4 style='margin:0; color:#00E676;'>CDSL (`CDSL.NS`) Fundamental Growth Thesis</h4>
